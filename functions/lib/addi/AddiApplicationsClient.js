...
export class AddiApplicationsClient {
    ...
    async createOnlineApplication(accessToken, payload) {
        ...
        const nested = json._links && typeof json._links === 'object'
            ? json._links.webRedirect?.href
            : undefined;
        const redirectUrl = (typeof json.redirectionUrl === 'string' && json.redirectionUrl) ||
        ...
        const applicationId = (typeof json.applicationId === 'string' && json.applicationId) ||
            (typeof json.id === 'string' && json.id) ||
            undefined;
        return { redirectUrl, applicationId };
    }
    async fetchCallbackCredentials(accessToken) {
        ...
        if (typeof json.user !== 'string' || typeof json.password !== 'string')
            return null;
        if (!json.user || !json.password)
            return null;
        return { user: json.user, password: json.password };
    }
    async fetchAllyAmountLimits(allySlug) {
