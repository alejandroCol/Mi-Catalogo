...
import { enrichLineasWithComboCost, validateCatalogLineStock, } from '../catalogInventoryFulfill.js';
import { isReservedStoreSlug } from '../storePublicUrl.js';
import { isTenantMembershipActive } from '../tenantMembership.js';
import { buscarCuponActivo, descuentoDesdeCupon, normalizeCuponCodigo, totalCheckoutCop, } from './catalogPricing.js';
 ...
 */
export async function buildAndPersistCatalogCheckoutOrder(db, input) {
    ...
    const carritoIniciadoId = typeof input.carritoIniciadoId === 'string' ? input.carritoIniciadoId.trim().slice(0, 128) : '';
    ...
    const destinatarioNombre = typeof input.destinatarioNombre === 'string' ? input.destinatarioNombre.trim().slice(0, 80) : '';
