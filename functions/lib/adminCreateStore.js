...
function validateSlug(raw) {
    ...
    if (!slug)
        return { ok: false, reason: 'slug_empty' };
...
async function resolveSlug(nombreTienda, customSlug) {
    if (customSlug?.trim()) {
        const v = validateSlug(customSlug);
        if (!v.ok) {
            const msg = v.reason === 'slug_taken'
...
async function loadPlatformTerms() {
    ...
    const version = typeof data?.platformTermsVersion === 'string' && data.platformTermsVersion.trim()
        ? data.platformTermsVersion.trim()
        : DEFAULT_TERMS_VERSION;
    const text = typeof data?.platformTermsText === 'string' && data.platformTermsText.trim()
        ? data.platformTermsText.trim()
        : '';
    return { version, text };
}
function hashTermsContent(text) {
    if (!text)
        return '00000000';
    let hash = 0x811c9dc5;
...
export const mcAdminCreateStore = onCall({ invoker: 'public' }, async (request) => {
    ...
    try {
        ...
        batch.set(tenantRef, {
            ...
            catalogPublishGrandfathered: false,
            productCount: 0,
