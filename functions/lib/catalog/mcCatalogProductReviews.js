...
async function resolveTenantId(slug) {
    ...
    return slugSnap.data().tenantId;
}
async function recomputeProductRating(tenantId, productId) {
...
async function uploadReviewImage(tenantId, reviewId, imageBase64) {
...
export const mcCatalogSubmitProductReview = onCall({ invoker: 'public', memory: '512MiB', timeoutSeconds: 60 }, async (request) => {
    ...
    const productNombre = String(productSnap.data().nombre || 'Producto').slice(0, 120);
    ...
    return { ok: true, reviewId, ...(imageUrl ? { imageUrl } : {}) };
});
...
export const mcCatalogModerateProductReview = onCall({ invoker: 'public' }, async (request) => {
    ...
    const tenantId = isSuper && typeof d.targetTenantId === 'string' && d.targetTenantId
        ? d.targetTenantId
        : user?.tenantId;
    if (!tenantId)
        throw new HttpsError('permission-denied', 'Sin tienda.');
    const ref = db.doc(`mc_tenants/${tenantId}/product_reviews/${reviewId}`);
    const snap = await ref.get();
    if (!snap.exists)
        throw new HttpsError('not-found', 'Reseña no encontrada.');
    const productId = String(snap.data().productId || '');
    if (!productId)
        throw new HttpsError('failed-precondition', 'Reseña inválida.');
    await ref.update({
        status,
        updatedAt: Date.now(),
