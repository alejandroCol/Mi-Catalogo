...
function normalizeItems(raw) {
    ...
    for (const row of raw) {
        ...
        const precioUnitarioCop = typeof precioRaw === 'number' && Number.isFinite(precioRaw) && precioRaw >= 0
            ? Math.round(precioRaw)
            : undefined;
        let cantidadDeseada = typeof r.cantidadDeseada === 'number' ? Math.floor(r.cantidadDeseada) : 1;
        if (!Number.isFinite(cantidadDeseada) || cantidadDeseada < 1)
            cantidadDeseada = 1;
...
function toPublicWishlist(id, data) {
    return {
        id,
        ...
        destinatarioTelefono: typeof data.destinatarioTelefono === 'string' ? data.destinatarioTelefono : undefined,
...
export const mcCatalogWishlistUpsert = onCall({ invoker: 'public' }, async (request) => {
    ...
    if (!titulo)
        throw new HttpsError('invalid-argument', 'Poné un título a tu lista.');
    if (!creadorNombre)
        throw new HttpsError('invalid-argument', 'Indicá tu nombre.');
    if (!destinatarioNombre)
        throw new HttpsError('invalid-argument', 'Indicá a quién le llega el regalo.');
    if (!envioDepartamento)
        throw new HttpsError('invalid-argument', 'Seleccioná el departamento de envío.');
    if (!envioCiudad)
        throw new HttpsError('invalid-argument', 'Indicá la ciudad o municipio.');
    if (!envioDireccion)
        throw new HttpsError('invalid-argument', 'Ingresá la dirección de envío.');
...
export const mcCatalogWishlistGet = onCall({ invoker: 'public' }, async (request) => {
    ...
    const canManage = sessionToken.length >= SESSION_MIN &&
...
export const mcCatalogWishlistRecordPurchase = onCall({ invoker: 'public' }, async (request) => {
    ...
    const nextItems = (list.items || []).map((it) => {
        ...
        for (const linea of orderLineas) {
            if (!linea.productId || linea.productId !== it.productId)
                continue;
            if (it.varianteId && linea.varianteId !== it.varianteId)
                continue;
            if (it.tallaId && linea.tallaId !== it.tallaId)
                continue;
            add += linea.cantidad;
        }
        if (add <= 0)
