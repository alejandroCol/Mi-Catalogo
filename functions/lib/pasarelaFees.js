...
function pasarelaTxBaseCommissionCop(grossCop) {
    const g = Math.max(0, Math.round(grossCop));
    return g * ONEPAY_MERCHANT_TX_RATE + ONEPAY_MERCHANT_TX_FIXED_COP;
}
export function pasarelaTxBaseFeePerPaymentCop(grossCop) {
    return Math.ceil(pasarelaTxBaseCommissionCop(grossCop));
}
export function pasarelaTxIvaOnCommissionCop(grossCop) {
    const base = pasarelaTxBaseCommissionCop(grossCop);
    return Math.ceil(base * ONEPAY_COMMISSION_IVA_RATE);
}
export function pasarelaTxFeePerPaymentCop(grossCop) {
    return pasarelaTxBaseFeePerPaymentCop(grossCop) + pasarelaTxIvaOnCommissionCop(grossCop);
}
export function pasarelaTxNetPerPaymentCop(grossCop) {
    const g = Math.max(0, Math.round(grossCop));
    return Math.max(0, g - pasarelaTxFeePerPaymentCop(g));
}
export function pasarelaMicatalogoFeePerPaymentCop(grossCop) {
    return pasarelaTxFeePerPaymentCop(grossCop);
}
export function pasarelaMicatalogoNetPerPaymentCop(grossCop) {
    return pasarelaTxNetPerPaymentCop(grossCop);
}
function pasarelaMicatalogoWithdrawalProportionalFeeCop(amountCop) {
    const a = Math.max(0, Math.round(amountCop));
    return Math.ceil(a * PASARELA_MICATALOGO_WITHDRAWAL_RATE);
}
export function pasarelaMicatalogoWithdrawalFeeCop(amountCop) {
    return (pasarelaMicatalogoWithdrawalProportionalFeeCop(amountCop) + PASARELA_MICATALOGO_WITHDRAWAL_FIXED_COP);
}
export function pasarelaMicatalogoNetAfterWithdrawalCop(amountCop) {
    const a = Math.max(0, Math.round(amountCop));
    return Math.max(0, a - pasarelaMicatalogoWithdrawalFeeCop(a));
}
export function onepayMerchantFeePerPaymentCop(grossCop) {
    return pasarelaTxFeePerPaymentCop(grossCop);
}
export function onepayMerchantNetPerPaymentCop(grossCop) {
    return pasarelaTxNetPerPaymentCop(grossCop);
}
