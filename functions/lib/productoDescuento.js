function productoDescuentoConfig(prod) {
    if (!prod.descuentoActivo || !prod.descuentoTipo || prod.descuentoValor == null)
        return null;
    const valor = Math.round(prod.descuentoValor);
    if (valor <= 0)
        return null;
    if (prod.descuentoTipo === 'porcentaje' && valor > 100)
        return null;
    return { tipo: prod.descuentoTipo, valor };
}
function descuentoMontoDesdePrecio(precioListaCop, tipo, valor) {
...
export function productoPrecioVentaFromData(prod) {
