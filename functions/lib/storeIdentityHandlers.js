...
function validateSlug(raw) {
    ...
    if (!slug)
        return { ok: false, code: 'slug_empty' };
    if (slug.length < SLUG_MIN)
        return { ok: false, code: 'slug_too_short' };
...
export const mcChangeStoreSlug = onCall({ invoker: 'public' }, async (request) => {
    ...
    const impersonateTenantId = typeof request.auth?.token?.[MC_IMPERSONATE_TENANT_CLAIM] === 'string'
    ...
    if (!oldSlug)
        throw new HttpsError('failed-precondition', 'Tu tienda no tiene dominio configurado.');
    if (newSlug === oldSlug) {
        return {
            ok: true,
            slug: newSlug,
    ...
    batch.set(db.doc(`mc_slugs/${oldSlug}`), {
        tenantId,
        active: false,
        updatedAt: now,
    }, { merge: true });
