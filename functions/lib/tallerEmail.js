...
function formatGoogleCalendarLocalDateTime(dateMs) {
...
function tallerEventPageUrl(slug) {
...
function buildGoogleCalendarUrl(taller, slug) {
    if (typeof taller.dateMs !== 'number' || Number.isNaN(taller.dateMs))
        return null;
...
function googleCalendarButtonHtml(taller, slug) {
...
function actionButtonsHtml(taller, slug, joinStyle) {
    const eventUrl = tallerEventPageUrl(slug);
...
function formatTallerDateLabel(dateMs) {
...
function buildTallerEmailShell(title, bodyHtml) {
...
function buildConfirmationEmailHtml(taller, attendeeName, slug) {
    const title = String(taller.title ?? 'Taller');
...
function buildReminderEmailHtml(taller, attendeeName, slug) {
    const title = String(taller.title ?? 'Taller');
...
async function assertMcSuperAdmin(uid) {
    if (!uid) {
...
function parseBrandType(raw) {
    const v = typeof raw === 'string' ? raw.trim() : '';
    if (v in BRAND_TYPE_LABELS)
        return v;
    return null;
}
