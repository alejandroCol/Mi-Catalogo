...
export function addiStripAccents(str: string): string {
  return str ? str.normalize('NFD').replace(/[\u0300-\u036f]/g, '') : ''
}

export function addiClip(str: string, max: number): string {
  return addiStripAccents(str).trim().slice(0, max)
}

export function addiCleanDigits(str: string): string {
  return String(str).replace(/\D/g, '')
}

export function addiCellphoneCo(raw: string): string {
...
export function addiCityCo(raw: string): string {
  let city = addiStripAccents(raw || 'Bogota').trim()
  if (city.toLowerCase().includes('bogota')) city = 'Bogota D.C'
  return city.slice(0, 60)
}

export function addiSplitName(fullName: string): { firstName: string; lastName: string } {
