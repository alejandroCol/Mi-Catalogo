...
export function buildAuthContinueUrl(publicOrigin: string): string {
...
export async function sendVerificationEmailWithResend(opts: {
  ...
  publicOrigin: string
  resendApiKey: string
}): Promise<{ ok: true } | { ok: false; error: string; firebaseCode?: string }> {
  ...
  try {
    ...
    const code = e && typeof e === 'object' && e !== null && 'code' in e ? String((e as { code: string }).code) : ''
