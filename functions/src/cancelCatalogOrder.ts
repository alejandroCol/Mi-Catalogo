...
import {
  onepayGetPaymentSnapshot,
  refundOnePayCatalogPayment,
  type OnePayRefundKind,
} from './onepayRefundApi.js'
...
async function resolveTenantForCatalogCancel(auth: {
  ...
  const user = userSnap.data() as { tenantId?: string; role?: string }
...
async function readOnePaySecretForOrder(
  tenantId: string,
  order: CatalogOrderDoc,
): Promise<string> {
  const credRef =
    order.onepayViaMicatalogo === true
      ? PLATFORM_ONEPAY_CRED_REF
      : db.doc(`mc_tenants/${tenantId}/private_onepay/credentials`)
  const snap = await credRef.get()
  const sk = (snap.data() as { secretKey?: string } | undefined)?.secretKey
...
function orderNeedsOnePayAction(order: CatalogOrderDoc): boolean {
  if (typeof order.onepayRefundedAt === 'number') return false
...
function assertPaymentBoundToOrder(params: {
  tenantId: string
  orderId: string
  totalCop: number | undefined
  viaMicatalogo: boolean
  payment: {
    metadata?: unknown
    externalId?: string
    amount?: number
  ...
  if (!storeId || storeId !== params.tenantId) {
    throw new HttpsError(
      'permission-denied',
      'Ese cobro OnePay no pertenece a esta tienda. No se puede devolver con la pasarela de Mi Catálogo.',
    )
  }
  if (!boundOrderId || boundOrderId !== params.orderId) {
    throw new HttpsError(
      'permission-denied',
      'Ese cobro OnePay no corresponde a este pedido.',
    )
  }
...
export const mcCancelCatalogOrder = onCall({ invoker: 'public' }, async (request) => {
  ...
  const d = request.data as { orderId?: unknown }
  const orderId = typeof d.orderId === 'string' ? d.orderId.trim() : ''
  if (!orderId) throw new HttpsError('invalid-argument', 'Falta el pedido.')
  ...
  const order = orderSnap.data() as CatalogOrderDoc

  let refundKind: OnePayRefundKind | null = null
  ...
  if (refundKind) {
    ...
    if (chargeIds.length > 0) patch.onepayRefundChargeIds = chargeIds
  }
