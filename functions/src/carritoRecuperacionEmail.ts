...
function generateRecoveryCouponCode(): string {
...
function buildRecoveryEmailHtml(opts: {
  ...
  cuponCodigo?: string
  colors: ReturnType<typeof resolveEmailCatalogThemeColors>
}): string {
  const c = opts.colors
  ...
  const discountBlock =
    opts.descuentoPorcentaje > 0 && opts.cuponCodigo
...
export async function sendCarritoRecuperacionEmail(opts: {
  ...
  descuentoPorcentaje: number
  cuponCodigo?: string
  tenant: { billingPlan?: string; catalogTheme?: { preset?: string; colors?: Record<string, string | undefined> } }
  ...
  try {
    ...
    const messageId = (r.data as { id?: string } | null | undefined)?.id?.trim()
    if (!messageId) {
...
function upsertRecoveryCouponInCatalog(
  cuponesCatalogo: McCuponTienda[] | undefined,
  carritoId: string,
  pct: number,
  existingCodigo?: string,
): { codigo: string; cuponesCatalogo: McCuponTienda[] } {
  const prev = Array.isArray(cuponesCatalogo) ? cuponesCatalogo : []
  const prevRecup = prev.find(
    (c) => c.esRecuperacion === true && c.carritoIniciadoId === carritoId,
  )
  const codigo = normalizeCuponCodigo(existingCodigo?.trim() || prevRecup?.codigo || generateRecoveryCouponCode())
  const cupon: McCuponTienda = {
...
function mapResendSendError(error: string): string {
...
export async function mcSendCarritoRecuperacionEmailHandler(
  db: Firestore,
  uid: string,
  dataRaw: unknown,
  resendApiKey: string,
  ...
  if (!resendApiKey) {
    throw new HttpsError(
      'failed-precondition',
      'Falta configurar RESEND_API_KEY para enviar correos de recuperación.',
    )
  }

  const data = (dataRaw && typeof dataRaw === 'object' ? dataRaw : {}) as {
    carritoId?: unknown
  ...
  const tenantId = (userSnap.data() as { tenantId?: string }).tenantId?.trim()
  if (!tenantId) {
  ...
  const subActive =
    typeof tenant.subscriptionEndsAt === 'number' && tenant.subscriptionEndsAt > Date.now()
  const inGrace =
    typeof tenant.billingGraceUntilMs === 'number' && tenant.billingGraceUntilMs > Date.now()
  ...
  const carrito = carritoSnap.data() as {
    estado?: string
    clienteEmail?: string
    clienteNombre?: string
    lineas?: CarritoLinea[]
  ...
  const recoveryUrl = buildStorePublicUrl(
    publicOrigin,
    slug,
    `/checkout?r=${encodeURIComponent(carritoId)}${pct > 0 && codigo ? `&cupon=${encodeURIComponent(codigo)}` : ''}`,
  )
