...
function formatCopEs(n: number): string {
...
function pickDefined(patch: Record<string, string | undefined> | undefined): Partial<ResolvedCatalogColors> {
  if (!patch) return {}
...
const FREE_PUBLIC_THEME: ResolvedCatalogColors = {
  accent: '#171717',
  accentText: '#fafaf9',
  bg: '#f4f3f0',
  surface: '#ffffff',
...
export function resolveEmailCatalogThemeColors(tenant: {
...
} | null | undefined): ResolvedCatalogColors {
  ...
  const preset: McCatalogThemePreset =
    rawPreset && rawPreset in PRESETS ? (rawPreset as McCatalogThemePreset) : 'morning'
  const base = PRESETS[preset]
  const custom = pickDefined(tenant?.catalogTheme?.colors as Record<string, string | undefined> | undefined)
  return { ...base, ...custom }
}
...
function buildEmailHtml(opts: {
  ...
  const clienteBits =
    opts.variant === 'customer'
      ? [
          opts.clienteNombre &&
            `<p style="margin:0 0 6px"><span style="color:${c.muted}">Nombre</span><br /><strong>${escapeHtml(
              opts.clienteNombre,
            )}</strong></p>`,
          opts.clienteTelefono &&
            `<p style="margin:0 0 6px"><span style="color:${c.muted}">Teléfono</span><br /><strong>${escapeHtml(
              opts.clienteTelefono,
            )}</strong></p>`,
          opts.envioCiudad &&
            `<p style="margin:0 0 6px"><span style="color:${c.muted}">Ciudad</span><br /><strong>${escapeHtml(
              opts.envioCiudad,
            )}</strong></p>`,
          opts.envioDireccion &&
            `<p style="margin:0"><span style="color:${c.muted}">Dirección de envío</span><br /><strong>${escapeHtml(
              opts.envioDireccion,
            )}</strong></p>`,
          opts.notaCliente &&
            `<p style="margin:12px 0 0"><span style="color:${c.muted}">Tu nota</span><br />${escapeHtml(
              opts.notaCliente,
            )}</p>`,
        ]
          .filter(Boolean)
  ...
  const trackingBox =
    opts.variant === 'customer' && opts.seguimientoUrl
  ...
  const cta =
    opts.variant === 'customer' && (opts.seguimientoUrl || opts.catalogUrl)
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:580px;background:${c.surface};border-radius:14px;overflow:hidden;border:1px solid ${mutedBorder};box-shadow:0 10px 40px rgba(15,23,42,0.06)">
          <tr>
            <td style="background:${c.accent};color:${c.accentText};padding:22px 26px 26px">
              <div style="font-size:10px;letter-spacing:0.12em;text-transform:uppercase;opacity:0.85;margin-bottom:8px">${escapeHtml(
                opts.nombreTienda,
              )}</div>
              <div style="font-size:22px;font-weight:700;line-height:1.25">${headline}</div>
              <div style="margin-top:10px;font-size:15px;opacity:0.95">Total · <strong>${formatCopEs(
                opts.totalCop,
              )}</strong></div>
            </td>
          </tr>
...
async function sendResendHtml(opts: {
  ...
  try {
    ...
    if (r.error) {
      return { ok: false, error: String((r.error as { message?: string }).message ?? r.error) }
    }
    return { ok: true }
  } catch (e) {
