...
export async function resolveLiveTenantForOwner(
  auth: { uid?: string; token?: Record<string, unknown> } | undefined,
): Promise<{ tenantId: string; tenant: LiveTenantSlice; uid: string }> {
  const uid = auth?.uid
  ...
  if (impersonateTenantId) {
    ...
    return { tenantId: impersonateTenantId, tenant: tenantSnap.data() as LiveTenantSlice, uid }
  }
  ...
  if (!tenantSnap.exists) throw new HttpsError('not-found', 'Tienda no encontrada.')
  return { tenantId, tenant: tenantSnap.data() as LiveTenantSlice, uid }
}

export function assertMasterLiveAccess(tenant: LiveTenantSlice): void {
...
export async function resolvePublicTenantBySlug(slug: string): Promise<{ tenantId: string; slug: string }> {
  const normalized = slug.trim().toLowerCase()
  if (!normalized) throw new HttpsError('invalid-argument', 'Slug inválido.')
