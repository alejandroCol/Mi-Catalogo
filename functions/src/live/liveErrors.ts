...
export class MuxStreamProviderError extends Error {
  ...
  readonly muxBody: string

  constructor(status: number, muxBody: string) {
...
function muxErrorMessage(body: string): string | null {
  try {
    const parsed = JSON.parse(body) as {
      error?: { messages?: string[]; type?: string }
    }
...
export function throwLiveServiceError(err: unknown): never {
  if (err instanceof HttpsError) throw err

  const message = err instanceof Error ? err.message : String(err)
  const muxDetail =
    err instanceof MuxStreamProviderError ? muxErrorMessage(err.muxBody) : null
  ...
  if (
    ...
    throw new HttpsError(
      'failed-precondition',
      'Tu cuenta de Mux está en plan gratuito y no permite live streams. Activá un plan de pago en mux.com (Mux Video) o usá modo demo: MC_LIVE_USE_MOCK=true en Functions.',
    )
  }
