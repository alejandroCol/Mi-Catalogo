...
async function onepayGeoGetJson(platformSk: string, url: string): Promise<unknown> {
  let res: Response
  try {
    res = await fetch(url, {
      headers: {
...
function normalizeStateRow(row: unknown): OnePayGeoState | null {
...
export function normalizeOnePayStatesPayload(raw: unknown): OnePayGeoState[] {
...
function normalizeCityRow(row: unknown): OnePayGeoCity | null {
  if (!row || typeof row !== 'object') return null
  const o = row as { id?: unknown; name?: unknown; state?: unknown }
  const id = typeof o.id === 'number' && Number.isFinite(o.id) ? o.id : parseInt(String(o.id ?? ''), 10)
  const name = typeof o.name === 'string' ? o.name.trim() : ''
  if (!Number.isFinite(id) || id <= 0 || name.length < 2) return null

  let state: OnePayGeoState | null = null
...
export function normalizeOnePayCitiesPayload(raw: unknown): OnePayCitiesPage {
  const empty: OnePayCitiesPage = {
    cities: [],
    current_page: 1,
    last_page: 1,
    per_page: 15,
...
export async function assertKybGeoCaller(request: CallableRequest): Promise<void> {
  if (!request.auth?.uid) {
...
export async function fetchOnePayStates(platformSk: string, filterName?: string): Promise<OnePayGeoState[]> {
...
export async function fetchOnePayCities(
  platformSk: string,
  opts: { stateId?: number; filterName?: string; page?: number; perPage?: number },
): Promise<OnePayCitiesPage> {
