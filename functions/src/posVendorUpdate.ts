...
async function assertStoreOwner(uid: string, tenantId: string): Promise<void> {
  ...
  const user = userSnap.data() as { isSuperAdmin?: boolean; tenantId?: string; role?: string }
  ...
  if (user.tenantId !== tenantId) throw new HttpsError('permission-denied', 'Sin acceso.')
  ...
  if (tenant.ownerUid !== uid) throw new HttpsError('permission-denied', 'Solo el dueño.')
}
...
export const mcUpdatePosVendor = onCall({ invoker: 'public' }, async (request) => {
  ...
  if (!tenantId || !vendorUid) throw new HttpsError('invalid-argument', 'Faltan datos.')
  await assertStoreOwner(uid, tenantId)

  const vendorRef = db.doc(`mc_users/${vendorUid}`)
  const vendorSnap = await vendorRef.get()
  if (!vendorSnap.exists) throw new HttpsError('not-found', 'Vendedor no encontrado.')
  ...
  return { ok: true as const, vendorUid }
})
