...
export const mcQuoteEnvioCheckout = onCall(
  { invoker: 'public', secrets: [enviaApiToken] },
  async (request) => {
    const data = (request.data && typeof request.data === 'object' ? request.data : {}) as {
      slug?: unknown
      envioDepartamento?: unknown
      envioCiudad?: unknown
      envioDireccion?: unknown
      destinoNombre?: unknown
      destinoTelefono?: unknown
      subtotalCop?: unknown
      totalPiezas?: unknown
    }
    ...
    const envioDepartamento =
      typeof data.envioDepartamento === 'string' ? data.envioDepartamento.trim() : ''
    ...
    const subtotalCop =
      typeof data.subtotalCop === 'number' && Number.isFinite(data.subtotalCop)
        ? Math.max(0, Math.round(data.subtotalCop))
        : 0
    const totalPiezas =
      typeof data.totalPiezas === 'number' && Number.isFinite(data.totalPiezas)
    ...
    const resolution = await resolveCheckoutEnvioCop({
      tenant: tenant as Parameters<typeof resolveCheckoutEnvioCop>[0]['tenant'],
      platform: platform as Parameters<typeof resolveCheckoutEnvioCop>[0]['platform'],
    ...
    const gratisUmbral =
      typeof (tenant as { envioGratisDesdeCop?: number }).envioGratisDesdeCop === 'number' &&
      (tenant as { envioGratisDesdeCop?: number }).envioGratisDesdeCop! > 0 &&
      subtotalCop >= Math.round((tenant as { envioGratisDesdeCop?: number }).envioGratisDesdeCop!)

    let lineaEnvio: 'oculta' | 'cobro' | 'gratis_umbral' | 'gratis_ciudad' | 'cotizacion' = 'oculta'
    if (gratisUmbral) {
