...
async function resolveActiveTenantBySlug(slug: string): Promise<{ tenantId: string }> {
  ...
  const endsAt = tenantSnap.data()?.subscriptionEndsAt
...
export async function recordStoreAnalyticsEvent(opts: {
  ...
  const sessionRef = db.doc(
    `mc_tenants/${tenantId}/analytics_sessions/${dateKey}_${opts.sessionId}`,
  )
  ...
  await db.runTransaction(async (tx) => {
    let productDailyRef: ReturnType<typeof db.doc> | null = null
    let productAggRef: ReturnType<typeof db.doc> | null = null

    if (productId) {
      productDailyRef = db.doc(
        `mc_tenants/${tenantId}/analytics_product_daily/${dateKey}__${productId}`,
      )
    ...
    if (productId && productDailyRef && productAggRef) {
      ...
      const aggPatch: Record<string, unknown> = {
        productId,
        productTitle,
        viewsTotal: Number(productAggSnap?.data()?.viewsTotal ?? 0) + 1,
        lastViewedAt: now,
      }
      if (productImageUrl) aggPatch.imageUrl = productImageUrl
...
export const mcRecordStoreAnalytics = onCall({ invoker: 'public' }, async (request) => {
  ...
  const event = request.data?.event as StoreAnalyticsEvent
