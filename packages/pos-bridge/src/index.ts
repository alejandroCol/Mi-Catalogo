...
function corsPos(req: Request, res: Response, next: NextFunction) {
...
async function runPs1(scriptName: string, args: string[]): Promise<{ stdout: string; stderr: string }> {
...
function parseWindowsPrintError(err: unknown, requested: string): string {
  const msg =
...
async function printRawWindows(printerName: string, data: Buffer): Promise<string> {
...
async function printRawUnix(printerName: string, data: Buffer): Promise<string> {
  const dir = mkdtempSync(join(tmpdir(), 'morning-pos-'))
  const file = join(dir, 'ticket.bin')
  writeFileSync(file, data)
...
async function printRaw(printerName: string, data: Buffer): Promise<string> {
